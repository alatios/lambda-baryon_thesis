...
finally, Section \ref{sec:lambda} will introduce the main topic of the thesis, the study of dipole moments of the $\Lambda^0$ baryon.
...
For this reason, measurements of MDMs are instead used as \textit{precision tests} of the CPT theorem, since its value should not change between a particle and its antimatter partner.
...
%% FONDAMENTALMENTE RIVEDI TUTTO!!!! che palle, però così è molto più leggibile.
...
\subsection{Initial polarization measurement}
...
The nature of LHCb as a forward detector implies that most $\Lambda^0$ baryons will fly along the laboratory frame $z$ axis, and therefore the initial polarization can be written as $\vec{s_0} = (0,0,s_0)$.

[Possibile mettere qui i frame of reference per discutere la rotazione indotta da questa situazione. Quella di Wick pertiene più alla misura della polarizzazione finale.]

[Altra opzione: prima fai initial e polarization measurements, poi parli delle non idealità. Insomma compatti le due sezioni in "Initial e final polarization measurements" e poi discuti i frame of reference.]

Equation \eqref{eq:spin-precession-l_cap1} for the $\Lambda^0$ spin precession after the magnetic field region can thus be simplified assuming a field $\vec{B} = (0,B_y,0)$:
\begin{equation}
\vec{s} =
\begin{cases}
	s_x = -s_0 \sin\Phi\\
...
[Discussione frame of reference?]

However, it must be noted that the assumption that $\vec{s}_0$ is directed along the laboratory frame $z$-axis demands a deal of approximation. First of all, while the 

[Qui va il discorso sulla rotazione di Wick, Sez. A.1.2.]

The LHCb tracking dipole magnet provides an integrated field $D_y \approx \pm 4 \,\si{\tesla\meter}$ \cite{LHCbDetectorPerformance}, allowing for a maximum precession angle of $\Phi_\text{max} \approx \pm \frac{\pi}{4}$ for $\Lambda^0$ traversing the entire region and reaching about 70\% of the maximum $s_y$ component in equation \eqref{eq:final_polarization_cap1}.


\begin{figure}[t!]
	\centering
	\includegraphics[scale=0.8]{graphics/01-standard_model/frames_of_reference.png}
	\caption[Helicity and laboratory frames of reference for the $\Lambda_b^0 \rightarrow J/\psi \, (\rightarrow \mu^+ \mu^-) \,\Lambda^0 \,(\rightarrow p \pi^-)$ decay.]{Frames of reference for the $\Lambda_b^0 \rightarrow J/\psi \, (\rightarrow \mu^+ \mu^-) \,\Lambda^0 \,(\rightarrow p \pi^-)$ decay: on the \textit{left} the $\Lambda^0$ helicity frame $\text{S}_{\Lambda\text{L}}$, on the \textit{right} the laboratory frame $\text{S}_{\text{L}}$.
...
\subsection{Final polarization measurement}
