...
The experimental layout is sketched in Figure \ref{fig:1:1971_cern_emdm_setup}: $\Lambda^0$ baryons are produced via the reaction
\begin{equation}
...
The production cross section peaks at \SI{0.8}{\milli\barn} and produced $\Lambda^0$ have $\approx 100\%$ normal polarization over a large angular region in the reaction reference frame.
The $\Lambda^0$ angular acceptance was restricted to $18^\circ \div 22^\circ$, near the maximum production angle of $21^\circ$, to amplify the signal-to-background ratio;
...
\end{equation}
with $\mu_N$ being the nuclear magneton \cite{1971_lambda_MDM}, while the $\Lambda^0$ EDM was measured at
\begin{equation}
...
finally, the $\Lambda^0 \rightarrow p\pi^-$ decay products within a defined volumetric acceptance are detected by the C1--6 multi-wire proportional chambers.
...
My work in this thesis aims to exploit the spin precession technique outlined in Section \ref{sec:emdm-measurement} to perform a measurement of the $\Lambda^0$ baryon electromagnetic dipole moments with the LHCb experiment at the Large Hadron Collider (see Chapter \ref{cap:LHCb}).
...
\cite{Richman:153636} \cite{PhysRev.108.1645}
that the expected angular distribution for said decay is
\begin{equation}
...
\begin{figure}[t]
	...
	\end{subfigure}
	\caption[Frames of reference for the \demonstratorfull decay.]{Frames of reference for the \demonstratorfull decay \cite{EMDipoleSearch}: \textit{(a)} the \lz helicity frame $\text{S}_{\Lambda\text{L}}$; \textit{(b)} the \lbz (heavy baryon) rest frame $\text{S}_{\text{H}}$; \textit{(c)} laboratory frame $\text{S}_{\text{L}}$.
...
\begin{itemize}
	\item the laboratory frame $\text{S}_\text{L}$ (Figure \ref{fig:1:frames_of_reference_lab}), with the $z$ axis along the proton beam and the $y$ axis along the vertical coordinate;
	\item the heavy baryon $\Lambda_b^0$ helicity frame $\text{S}_H$ (Figure \ref{fig:1:frames_of_reference_heavy}), with the $z$ axis given by the $\Lambda_b^0$ momentum in $\text{S}_\text{L}$ and the $x$ axis parallel to the normal to its production plane;
	\item the two $\Lambda^0$ helicity frames $\text{S}_\Lambda$ and $\text{S}_{\Lambda \text{L}}$ (Figure \ref{fig:1:frames_of_reference_lambda}). These are functionally the same frame of reference, the key difference being that the $z$ axis is defined in the direction of the \lz momentum in $\text{S}_H$ and $\text{S}_\text{L}$ respectively.
\end{itemize}
...
This effect arises in the case of \lz not directed along the $\text{S}_\text{L}$ $z$ axis and is expected to be negligible for the purposes of this thesis.

\begin{figure}[t]
	\centering
	\includegraphics[scale=0.7]{graphics/01-standard_model/heavy-baryon-production.pdf}
	\caption[Diagram of $\Lambda^0_b$ and \lz production at LHCb.]{Ioboh.}
	\label{fig:heavy-baryon-production}
\end{figure}

More significant is the Wick rotation, owing to the orientation discrepancy between $\text{S}_\Lambda$ frame (where the \lz has the maximal longitudinal polarization)  and $\text{S}_{\Lambda \text{L}}$ (where the angular distribution of \lz decay products is measured).
This phenomenon introduces a dilution effect to the precession measurement: a \lz with polarization $\vec{s_0} = s_0 \hat{z}_\Lambda$ in the $\text{S}_\Lambda$ frame gains in the $\text{S}_{\Lambda\text{L}}$ frame a transverse component of magnitude $s_0 \sin\alpha$, where \cite{spinInParticlePhysics}
\begin{equation}
\sin\alpha = \frac{m_\Lambda}{m_H} \frac{\left\lvert \vec{p}_H^{(\text{L})} \right\lvert}{\left\lvert \vec{p}_\Lambda^{(\text{L})} \right\lvert}
\end{equation}
and $\theta$ is the \lz helicity angle, i.e. the angle formed by the \lz momentum in $\text{S}_H$ with respect to the frame $z_H$ axis.

[@todo: Rotazione di Wick]
...
%% Non so se aggiungere sensitivity studies. Non mi pare sia il nostro caso! Al massimo si possono mettere prima.