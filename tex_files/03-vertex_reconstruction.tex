...
\subsection{Oscillating behaviour through VF iterations}
\begin{figure}[t]
	\centering
...
\subsection{Proton momentum discrepancy at first measurement position}
\begin{figure}[t]
	\centering
...
\begin{table}[t]
	...
	\begin{tabular}{|c||c|c|c|c|}
		\hline
		Increased $\sigma$ & Recovery power & $\mu_\frac{1}{2}\left[\tilde{\chi}^2\right]$ & $\mu_\frac{1}{2}\left[z_\Lambda^\text{vtx} \text{ bias}\right]$ & $\mu_\frac{1}{2}\left[p_z^\text{DTF} (p) \text{ bias}\right]$ \\
		\hline
		\hline
	...
	\end{tabular}
	\end{center}
	\caption{Performance comparison of the three rescaled-$\sigma$ algorithms (see text for details) with a $2\%$ increase in the respective vertex position uncertainties, contrasted with the performance of the standard Vertex Fitter algorithm. \textit{Recovery power} is defined as the ratio of events reconstructed by a certain flavour over the total number of events recoverable by combining the three algorithms. $\mu_\frac{1}{2}$ identifies the median value. Performances for $xyz$ algorithms are computed using all events recovered by the individual flavour, including events recovered by more than one, while values for VF are computed on standard reconstructed events.}
\end{table}