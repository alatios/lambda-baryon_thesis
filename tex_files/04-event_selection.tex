...
a sequence of decision nodes applies binary conditions according to available information on the individual event and eventually reaches a leaf node associated to the \textit{event score}, in this case the probability that the event is a \demonstratorshort decay.
...
Figure \ref{fig:4:confusion_matrix} shows the confusion matrices for training and test data with a score threshold of $s_\text{thres}=0.5$, meaning events with $s>s_\text{thres}$ are classified as signal.
...
[Devi dire che usi DTF con doppio m.c.]

First, the simulated signal $m(J/\psi~\Lambda^0)$ distribution was fit with a double-tailed asymmetric Crystal Ball function.
A standard, single-tailed Crystal Ball probability density function is parameterized as
\begin{equation}
...
signal is modeled by the same double-tailed Crystal Ball from \eqref{eq:4:crystal_ball}, with all parameters bar $S$ fixed to their Monte Carlo fit best values;
...
As will be shown in Section \ref{sec:4:performance_data}, the outlined combination of $f_\text{sig}$ and $f_\text{bkg}$ is a great fit for Run 2 data, as long as sufficient filtering was applied for the $\Lambda_b^0$ resonance peak to emerge.
