...
\end{equation}
with $\vec{r}_k$ being the reference point from representation \eqref{eq:particle_representation}.
This vertex doubles as reference point for the $k$-th particle until the next transportation, ergo track momentum is also updated to match it\footnote{In this equation, $\coloneqq$ stands for the assignment operator.}
\begin{equation}
	\vec{q}_k \coloneqq
	\vec{q}_k
	-
...
As one can observe, the recursive procedure described above requires, at each step, a previous estimated vertex position $\vec{x}_{k-1}$, an associated inverse covariance matrix $C_{k-1}^{-1}$ and a $\chi^2_{k-1}$. In particular, step $k=1$ demands the existence of $\vec{x}_0$, $C_{0}^{-1}$ and $\chi^2_{0}$.
...
\begin{align}
	{\chi^2_0}_{(1)} &= 
	{\left(\vec{r}_{1} - \vec{x}_0\right)}^T  {V_r}_1^{-1} \left(\vec{r}_{1} - \vec{x}_0 \right), \\
	{\chi^2_0}_{(1)} &= 
	{\left(\vec{r}_{2} - \vec{x}_0\right)}^T  {V_r}_2^{-1} \left(\vec{r}_{2} - \vec{x}_0 \right),
\end{align}
\end{subequations}
conventionally choosing $\chi^2_0 = {\chi^2_0}_{(1)}$.
...
Available information does not distinguish between the \lz and \lbz vertexing efficiencies\footnote{\jpsi vertex reconstruction is is a prerequisite for event selection in the detached $J/\psi\rightarrow \mu^+ \mu^-$ trigger line (see Section \ref{sec:2:used_data}) and does not factor in the vertexing efficiency}.
Nevertheless, the rare usage of T tracks for physics analysis in LHCb suggests that problems are likelier to arise in the \lambdadecay vertexing and then cascade into the \demonstratorshort reconstruction.
...
No difference between converged and failed \demonstratorshort events emerged at Monte Carlo level when considering the true values of basic kinematic variables such as daughter particle momenta and decay vertices locations of the three unstable particles \lbz, \lz and \jpsi.
...
In spite of such a discrepancy, the standard modus operandi for kine\-matics-at-vertex analyses usually compares true momenta (evaluated at the true vertex) with reconstructed momenta (evaluated at the reconstructed vertex).
...
This can potentially result in the conflict in $xz$ and $yz$ topologies observed in non-converged events, the current leading hypothesis to explain the oscillatory behaviour of the VF algorithm which prevents decay convergence.
...
\begin{enumerate}
	\item $C^{-1}_{k-1}$, the inverse $\vec{x}$ covariance matrix computed at the previous step (or previous iteration);
	\item ${V_r}_k^{-1}$, the inverse covariance matrix of reference point $\vec{r}_k$, computed at the true transport of particle $k$;
	\item $C_k$, the current $\vec{x}$ covariance matrix, inverted from the matrix sum of the previous two terms as in \eqref{eq:3:VF_new_inv_covmatrix}.
\end{enumerate}
...
Before proceeding, however, a digression is in order to better understand the meaning of \textit{performance} when comparing the algorithms.
...
for this subsample algorithm performances show very little variation, with marginal performance hierarchy of $\text{VF} > \sigma_x > \sigma_y > \sigma_z$.
...
Where algorithm performances truly diverge is in \textit{esclusive} events, i.e. events only reconstructed by one particular algorithm.
...
The overall $\tilde{\chi}^2_\text{vtx}$ of the \lambdadecay vertex for recovered events is significantly worse than the expect 1.0 value obtained by the VF;
...
This increased bias is a consequence of the systematic T track $p_z$ underestimation discussed in Section \ref{sec:3:true_vtx_kinematics}, which is also reflected in the negative $p_z$ bias computed by the Decay Tree Fitter algorithm with \jpsi and \lz mass constraints on recovered events.
...
Since both $x_\text{vtx}^\Lambda$ and $y_\text{vtx}^\Lambda$ are fairly well reconstructed, with resolution $\lesssim \SI{1}{\centi\meter}$ and no discernible bias, the main focus will be on the reconstruction of $z_\text{vtx}^\Lambda$.
...
\begin{figure}[t]
	\centering
	\includegraphics[width=.6\textwidth]{graphics/03-vertex_reconstruction/lambda_endvertex_z_bias_vs_ghost_vertex.pdf}
	\caption{Normalized distribution of $z_\text{vtx}^\Lambda$ bias for simulated \demonstratorshort events without (\textit{horizontal hatching}) and with (\textit{diagonal hatching}) \lz ghost vertex, as selected in Figure \ref{fig:3:psi_vs_htrue}. All selection steps are applied.}
