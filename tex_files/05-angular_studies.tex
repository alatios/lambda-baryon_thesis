...
\begin{itemize}
	...
	\item experimental resolutions on \cthetap and \phip are averaged over the respective variable ranges;
...
Since the Gaussian smearing also tends to flatten the \phip distribution, a fit to the measured proton angular distribution will result in a systematic underestimation of $g$ unless the fit function takes due account of the resolution, e.g. via \phip-convolution with a Gaussian distribution.
...
Figure \ref{fig:5:bw_residuals} tackles the same resolution comparison with a more numerical approach, plotting the binwise percentage difference between smeared and non-smeared angular distributions.
