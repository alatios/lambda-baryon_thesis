...
The three T stations, labeled as T1--3, are the last line of defense for LHCb tracking purposes, covering the $z \approx 7.7$--$9.4\,\si{\meter}$ region downstream of the dipole magnet \cite{Barbosa-Marinho:582793}.
...
\begin{itemize}
	...
	\item \textit{VELO} tracks are reconstructed from hits in the VELO detector alone. These are often large-angle or backward tracks, valuable to correctly identify the event primary vertex.
	\item \textit{Downstream} tracks are reconstructed from hits in the TT and T1--3 detectors. This is the go-to category to study long-lived particles (LLP) decaying after the VELO, such as $\Lambda^0$ baryons and $K_S^0$ mesons.
	\item \textit{T} tracks are only reconstructed from hits in the three T stations. Since missing both VELO and TT is a rare occurrence, most of these tracks come from LLPs with $c\tau \gtrsim \SI{5}{\meter}$ decaying after the dipole magnet.
\end{itemize}
Sketches of tracks satisfying the above requirements are depicted in Figure \ref{fig:2:track_classification}.

Among the four, T tracks have definitely seen the least use over LHC Runs 1 and 2.
The main reason is related to their key property of only having measured constraints after the magnet:
in order to reconstruct their origin vertex, T tracks have to be extrapolated through several meters of intense magnetic field, an operation that has ripercussions both in terms of accuracy and in timing.
On top of that, the further a charged particle is produced after the dipole magnet, the less the residual magnetic field will be able to imprint a significant bending radius, tanking the momentum resolution at T station level.
Over the ten years of detector operation there has been little physics incentive to fix or mitigate these issues, since most LLPs relevant to LHCb can already be studied using Downstream tracks.

Regardless of the above, T tracks are essential for the $\Lambda^0$ EDM/MDM measurement proposal outlined in Section \ref{sec:lambda}, hinging on the spin precession of the baryon traversing the magnetic field before decaying in the $p\pi^-$ pair.
Over the course of the following chapters, I'll delve into more detail on the approaches I have adopted to overcome the problems associated with T tracks in order to extract competitive measurements of the electromagnetic dipole moments.
...
Stations M2--M5 are placed downstream of the calorimeter system, separated between each other by \SI{80}{\centi\meter} of iron;
...
The EFF upgrade conducted during the LS1 increased the HLT2 allotted computation time enough to match the two performances;
...
As final step, the simulated hits from the virtual detector in GEANT4 are digitized using the Boole application, which aims to mimic the real output from the LHCb detector.
...
After the performance match between HLT2 and Brunel achieved for Run 2, the high level trigger will see an even more central stage in Run 3, being tasked with full reconstruction of all events of interest in the context of a five-fold increase in operational luminosity (up to \SI{2e33}{\per\centi\meter\squared\per\second}).
...
Upstream of the magnet, the Upstream Tracker (UT) will take over from the TT, featuring four high granularity silicon microstrip layers with improved coverage of the LHCb angular acceptance. Finally, both inner and outer trackers of the T1--3 stations will be replaced by the Scintillating Fiber Tracker (SFT or SciFi), based on \SI{2.4}{\meter} plastic scintillating fibers read out by silicon photo-multipliers \cite{Collaboration:1647400}.
...
The need to measure spin precession in the $B$ field restricts the available $\Lambda^0$ to those decaying after the LHCb dipole magnet, implicitly requiring its decay products $p\pi^-$ to be T tracks.

The $\tau \approx \SI{1.5e-12}{\second}$ mean life of the $\Lambda_b^0$ \cite{PDG} places its decay vertex well inside the VELO detector; in conjunction with the high efficiency reconstruction of muons in LHCb, this justifies a Long track requirement in their case.
The $\Lambda_b^0$ vertex accuracy afforded by the $J/\psi$ half of the decay chain enables us to impose kinematic constraints on the $\Lambda^0$ half, partially offsetting some of the problems with T tracks I have pointed out in Section \ref{sec:2:tracking}.
...
These have been generated using \textsc{Pythia} \cite{Pythia2015} using the LHCb-specific configuration implemented in Gauss \cite{Belyaev_2011}, and have been digitized following the procedure detailed in Section \ref{sec:2:monte_carlo}.