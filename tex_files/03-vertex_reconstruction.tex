...
As proof of concept, I have analyzed the performance of the $\sigma$-rescaled refit approach on a sample of [@todo:how many] MC-generated $\Lambda_b^0 \rightarrow J/\psi~\Lambda^0$ events, observing a $+25\%$ increase in reconstructed events.
As per Figure \ref{fig:lambda_lambdab_reco_efficiency}, this corresponds to about a quarter of all reconstructible events.

%% @todo: aggiungi xyz sequential
\begin{table}[t]
	\begin{center}
...
Defining the \textit{recovery efficiency} of each algorithm as the fraction of recovered events converging under said algorithm, i.e.
\begin{equation}
	\varepsilon^\text{reco}_i i\in{x,y,z}
\end{equation}
...
The extent of $z_\Lambda^\text{vtx}$ bias is therefore to be understood in terms of what events produce a convergence from a specific algorithm, rather than as performance evaluation of the algorithm itself.
The $\sigma_z$-rescaled algorithm does not intrinsically reconstruct events with a larger bias;
instead, borrowing the poor measurement hypothesis from Section \ref{sec:3:true_vtx_kinematics}, its $xy$-centric approach is better suited to find convergence in events with poor protoparticle reconstruction, which tend to have a larger $z_\Lambda^\text{vtx}$ bias on their best vertex candidate.
