...
%% Rifai meglio: non serve dedurre qui che L cambia in un certo modo, anche perché poi con E non lo fai.
...
From the above formula it should thus be immediately apparent that $\vec{L}$, $\vec{S}$, $\vec{s}$ and $\vec{\delta}$ are all polar vectors, i.e. parity-even.
By contrast, with a bit more work one can show that the electric field $\vec{E}$ is an axial vector, i.e. parity-odd, which makes $H_\text{EDM}$ a parity-odd pseudoscalar:
\begin{equation}
