...
its mass of $\approx 1116\,\text{MeV}/c$ \cite{PDG}, the lighest among $s$-bearing baryons, meant its only viable decay channels were mediated by the flavour-changing weak interaction, giving the $\Lambda$ a much longer half-life than expected;
...
The $\Lambda$ baryon is a prime candidate to probe CP violation.
...
\subsection{Proposal of a measurement of the \texorpdfstring{$\Lambda^0$}{Lambda} dipole moments with the LHCb detector}

The properties of the LHCb detector at the Large Hadron Collider (see Chapter \ref{cap:LHCb}) are a perfect fit for the measurement of the $\Lambda$ baryon electromagnetic dipole moments via the spin precession technique outlined in Section \ref{sec:emdm-measurement}.

Gradient field effects for the $\vec{B}$ field within the detector acceptance are negligible, being estimated at \cite{EMDipoleSearch}
\begin{equation}
\frac{\hbar}{2mc} \frac{\beta\gamma}{\gamma+1} \frac{|\nabla B|}{B} \approx 7.4 \times {10}^{-16},
\end{equation}
with $B \coloneqq |\vec{B}|$.
As well as being a neutral particle, thereby lacking the Thomas precession effect omitted from the discussion in Section \ref{sec:emdm-measurement}, the $\Lambda$ baryon's average mean life of $\approx 2.6 \times {10}^{-10}$ s \cite{PDG} allows a sizeable number of them to traverse the full LHCb magnetic field region before decaying.
Finally, the problem concerning the measurement of the $\Lambda^0$ initial polarization is solved by selecting $\Lambda^0$ produced through the weak decay production of bottom baryon $\Lambda_b^0$
\begin{equation}
\Lambda_b^0 \rightarrow J/\psi \, (\rightarrow \mu^+ \mu^-) \,\Lambda^0 \,(\rightarrow p \pi^-),
\label{eq:demonstrator_cap1}  
\end{equation}
as well as its charge-conjugate
\begin{equation}
\bar{\Lambda}_b^0 \rightarrow J/\psi \, (\rightarrow \mu^+ \mu^-) \,\bar{\Lambda}^0 \, (\rightarrow \bar{p} \pi^+).
\label{eq:antidemonstrator_cap1}  
\end{equation}
Parity violation in this decay produces $\Lambda^0$ with almost $100\%$ longitudinal polarization \cite{ATLASLambdaPolarization}, meaning that the original polarization can be inferred from the $\Lambda^0$ reconstructed momentum.

[Come si riduce l'equazione della spin polarization nel caso del barione lambda polarizzato.]
