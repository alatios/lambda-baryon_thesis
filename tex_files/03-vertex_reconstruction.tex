...
%% Usato FixJPsi perché altrimenti devi incorporare il vincolo DTF e che palle.
%% @todo: devi rirunnare individualmente gli eventi e confrontarli! I sigma-singoli, in pratica, VF e Sequential li ho aggiornati.
\begin{table}[t]
	\begin{center}
	\begin{tabular}{@{}lllll@{}}
		\toprule
		Increased $\sigma$ & Recovery efficiency & Median $\tilde{\chi}^2_\text{vtx}(\Lambda^0)$ & \multicolumn{2}{c}{Median bias} \\
		\cmidrule{4-5}
		&&& $z_\text{vtx}^\Lambda$ [mm] & $p_z^\text{DTF} (p)$ \\
		\midrule
		None (VF)  & -- & 1.0 & 399 & $+0.02\%$ \\
		$\sigma_x$ &  &  &  &  \\
		$\sigma_y$ &  &  &  &  \\
		$\sigma_z$ &  &  &  &  \\
		Sequential & 100\% (definition) & 7.1 & 604 & $-0.81\%$ \\
		\bottomrule
	\end{tabular}
	\end{center}
